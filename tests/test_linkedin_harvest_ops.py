...
from src import persistence_runtime_ops as ops
...
class HarvestOpsTests(unittest.TestCase):
    ...
    def test_install_continuous_hourly_harvest_uses_runtime_entrypoint_and_config_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            run_script = root / "src" / "runtime_entrypoints.py"
            ...
            self.assertEqual(
                (
                    f"0 * * * * /abs/python3.11 {tool._paths.run_script_path} run-harvest-cron >> {tool._paths.cron_log_path} 2>&1",
                ),
                block.entries,
            )
            self.assertIn(str(tool._paths.schedule_override_path), output.getvalue())

    def test_remove_harvest_reports_empty_crontab(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            run_script = Path(temp_dir) / "src" / "runtime_entrypoints.py"
            run_script.parent.mkdir(parents=True, exist_ok=True)
            run_script.write_text("#!/usr/bin/env python3\n", encoding="utf-8")
            tool = ops.HarvestCronScripts(run_script)
            tool._crontab = FakeCrontabManager()
    ...
    def test_run_once_clears_pid_file_after_subprocess_finishes(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("src.persistence_runtime_ops.subprocess.run", return_value=SimpleNamespace(returncode=7)) as mock_run, patch.object(ops, "_python_executable", return_value="/abs/python3.11"):
    ...
    def test_harvest_status_reports_not_running_for_stale_pid_file(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            script_path = Path(temp_dir) / "src" / "runtime_entrypoints.py"
            ...
            output = io.StringIO()
            with patch("src.persistence_runtime_ops._pid_is_running", return_value=False), contextlib.redirect_stdout(output):
