...
- Ingestion flow wiring is in `src/runtime_entrypoints.py`:
  ...
  - fetch via source adapter (`src/linkedin_acquisition.py`)
...
- Deduplication rule is `JobRecord.dedupe_key`:
  ...
  - fallback to normalized link (`src.core_domain_inputs.normalize_source_link` strips query string and trailing slash)
